...
const outDir = path.join(process.cwd(), "audit-results", "v971-screenshots");
...
const report = [
  "# V9.7.1 Visual Density Taskuri Workspace Screenshot Audit Report",
  "",
  `BaseUrl: ${baseUrl}`,
  "",
...
const reportPath = path.join(process.cwd(), "audit-results", "V9_7_1_SCREENSHOT_AUDIT_REPORT.md");
fs.writeFileSync(reportPath, report.join("\n"), "utf8");
if (passed !== routes.length) {
  throw new Error(`v9.7.1 screenshot audit failed: ${passed} / ${routes.length}. Report: ${reportPath}`);
}
console.log(`v9.7.1 screenshot audit captured clean: ${passed} / ${routes.length}`);
