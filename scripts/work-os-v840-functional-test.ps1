...
$routes = @(
  ...
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/health",
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/adapter-execution",
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/dispatch-worker",
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/dead-letter",
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/runtime-proof",
  "/api/v1/work-os/v84-database-adapter-dispatch-worker/rollback-worker"
)

$report = @()
$report += "# v8.4.0 Database Adapter Dispatch Worker Functional Route/API Test"
$report += ""
...
$reportPath = Join-Path (Join-Path (Get-Location) "audit-results") "v840-database-adapter-dispatch-worker-functional-routes.md"
$report -join "`n" | Set-Content -Path $reportPath -Encoding UTF8
Write-Host "v8.4.0 functional route/API smoke passed: $passed / $($routes.Count)"
