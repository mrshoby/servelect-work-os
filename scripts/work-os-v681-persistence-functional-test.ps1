...
$ReportPath = Join-Path $OutDir "v681-persistence-api-functional-routes.md"
...
$Header = @(
    "# SERVELECT WORK OS v6.8.1 Persistence/API Functional Route Smoke",
    "",
    "Base URL: $BaseUrl",
...
if ($Failed -gt 0) {
    throw "v6.8.1 persistence/API route smoke failed: $Failed route(s). See $ReportPath"
}

Write-Host "v6.8.1 persistence/API route smoke passed: $($Routes.Count) / $($Routes.Count)" -ForegroundColor Green
