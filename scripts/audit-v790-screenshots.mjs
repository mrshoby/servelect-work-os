...
const outDir = path.join(process.cwd(), "audit-results", "v790-screenshots");
...
for (const route of routes) {
  ...
  try {
    ...
    if (status === 401 && baseUrl.includes("vercel.app") && !bypassSecret) { rows.push({ route, result: allowProtected401 ? "PROTECTED_401" : "FAIL_PROTECTED_401", http: 401, png: "NO_PNG", bytes: 0, note: "Vercel Deployment Protection returned 401 before app render." }); continue; }
    if (!response || status >= 400) { rows.push({ route, result: `FAIL_HTTP_${status}`, http: status, png: "NO_PNG", bytes: 0, note: "HTTP failure" }); continue; }
    await page.screenshot({ path: target, fullPage: true });
    const stat = await fs.stat(target);
    rows.push({ route, result: "PASS", http: status, png: file, bytes: stat.size, note: "OK" });
  } catch (error) { rows.push({ route, result: `FAIL: ${error instanceof Error ? error.message : String(error)}`, http: 0, png: "NO_PNG", bytes: 0, note: "Capture failed" }); }
}
await browser.close();
