...
$report = @()
$report += "# v9.7.0 Portfolio WorkGraph Reporting Functional Route/API Test"
$report += ""
...
$path = Join-Path $dir "v970-portfolio-workgraph-reporting-functional-routes.md"
$report | Set-Content -Path $path -Encoding UTF8

if ($passed -ne $routes.Count) {
  throw "v9.7.0 functional route/API smoke failed: $passed / $($routes.Count). Report: $path"
}
Write-Host "v9.7.0 Portfolio WorkGraph Reporting smoke passed: $passed / $($routes.Count)"
