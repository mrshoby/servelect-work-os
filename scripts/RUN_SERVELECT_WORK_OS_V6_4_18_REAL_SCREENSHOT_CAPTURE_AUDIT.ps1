...
$OutputDir = Join-Path $env:USERPROFILE "Downloads\SERVELECT_WORK_OS_v6.4.18_REAL_SCREENSHOTS"
...
function Capture-Route {
  ...
  # Folosim string cu apostrof pentru a evita parser error PowerShell cauzat de ghilimele escapate in regex.
...
Write-Host ""
Write-Host "=== v6.4.18 screenshot capture audit ==="
...
try {
  ...
  $report = @(
    "# SERVELECT WORK OS v6.4.18 real screenshot capture audit",
    "",
    "Browser: $Browser",
    "BaseUrl: $BaseUrl",
    "OutputDir: $OutputDir",
