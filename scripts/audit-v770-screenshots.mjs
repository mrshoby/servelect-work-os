...
for (const route of routes) {
  ...
  try {
    ...
    if (status === 401 && baseUrl.includes("vercel.app") && !bypassSecret) {
      rows.push({
        route,
        result: allowProtected401 ? "PROTECTED_401" : "FAIL_PROTECTED_401",
        http: 401,
        png: "NO_PNG",
        bytes: 0,
        note: "Vercel Deployment Protection / Vercel Authentication returned 401 before app render. Set VERCEL_AUTOMATION_BYPASS_SECRET or test an unprotected alias."
      });
      continue;
    }
...
const report = [
  "# V7.7.0 / V7.7.3 Screenshot Audit Report",
  "",
  `BaseUrl: ${baseUrl}`,
