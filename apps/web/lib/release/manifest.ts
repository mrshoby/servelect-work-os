...
import { V75_RELEASE_VERSION, v75CurrentReadiness, v75GlobalScores, v75ProgressScores, v75RouteList } from "@/lib/enterprise/work-os-v75-conflict-access-attachments";
...
export const SERVELECT_RELEASE_VERSION = V75_RELEASE_VERSION;
export const SERVELECT_RELEASE_CHANNEL = "v7.5.1 Conflict Resolution, Access Inheritance & Real Attachment Storage";
...
export const releaseMilestones: ReleaseMilestone[] = [
  ...
  { id: "v75", title: "Conflict Resolution, Access Inheritance & Real Attachment Storage", version: V75_RELEASE_VERSION, date: "2026-06-12", summary: "Conflict merge UI, access inheritance and R2/S3-ready attachment metadata while primary writes remain gated.", routes: v75RouteList() }
];
...
export const releaseGates: ReleaseGate[] = [
  ...
  { id: "v74-screenshots", title: "v7.4.0 screenshot baseline", status: "passed", owner: "QA", evidence: "V7.4.0 screenshot report confirmed 11/11 PNG on Vercel.", action: "Extend screenshot set with v7.5 routes." },
  { id: "conflict-resolution", title: "Conflict resolution", status: "passed", owner: "Platform", evidence: "v7.5.1 adds open conflict records with merge/local/remote decisions.", action: "Connect merge operations to richer task drawer field UI." },
  { id: "access-inheritance", title: "Access inheritance", status: "passed", owner: "Security", evidence: "v7.5.1 adds workspace/project/task/ticket inherited ACL rules.", action: "Enforce inherited rules in all mutation endpoints." },
  { id: "attachment-storage", title: "Attachment storage", status: "warning", owner: "Platform", evidence: "R2/S3-ready metadata exists; signed URL binary upload is next.", action: "Add signed upload/download URL provider." },
...
export function getReleaseManifest() {
  ...
  const routes = [...v70RouteList(), ...v71RouteList(), ...v72RouteList(), ...v73RouteList(), ...v74RouteList(), ...v75RouteList()];
  const scores = v75ProgressScores();
  const global = v75GlobalScores();
  return {
    ok: checklist.blockers.length === 0,
    generatedAt: new Date().toISOString(),
    app: { name: "SERVELECT WORK OS / SERVELECT EMP", version: SERVELECT_RELEASE_VERSION, channel: SERVELECT_RELEASE_CHANNEL, releaseType: "conflict-access-attachments" },
    ...
    previousProgressScores: v74ProgressScores(),
    ...
    globalScores: global,
    previousGlobalScores: v74GlobalScores(),
    previousGlobalScoresV73: v73GlobalScores(),
    previousGlobalScoresV72: v72GlobalScores(),
    v71GlobalScores: v71GlobalScores(),
    readiness: v75CurrentReadiness(),
    previousReadiness: v74CurrentReadiness(),
    previousReadinessV73: v73CurrentReadiness(),
    previousReadinessV72: v72CurrentReadiness(),
    nextRecommendedVersions: [{ version: "7.6.0", title: "Signed Attachment URLs, Provider Delivery & Access-Enforced Mutation API", focus: "Add signed R2/S3 URLs, notification provider switchboard and enforce inherited access in mutation endpoints." }]
  };
}
