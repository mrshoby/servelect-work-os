...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, version: V71_RELEASE_VERSION, records: snapshot.state.tasks, mode: "api_shadow" });
}

export async function POST(request: Request) {
  const body = await request.json().catch(() => ({}));
  const mutation = buildMutationRequest("task", String(body.action ?? "update") as never, (body.payload ?? body) as Record<string, unknown>, String(body.actorId ?? "u_andrei"), String(body.role ?? "Super Admin"), String(body.department ?? "Management"), "api_shadow");
