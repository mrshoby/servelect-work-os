...
export default function TasksPage() {
  ...
  const dueToday = useMemo(
    () => tasks.filter((task) => task.dueDate <= "2024-05-18" && task.status !== "Finalizat"),
    [tasks]
  );
  ...
  return (
    <>
      <PageHeader
        title="Taskuri"
        subtitle="Task Center optimizat: tabel, board, calendar, approvals și workload fără blocaje în browser."
        actions={
          <>
            {timerTaskId && (
        ...
        <div className="grid gap-0 xl:grid-cols-[1fr_360px]">
          <div className="p-6 lg:p-7">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
              Taskurile sunt legate de proiecte, responsabili, statusuri, deadline-uri, estimări, comentarii și checklist.
              Versiunea aceasta păstrează interfața, dar randează doar view-ul activ pentru performanță.
            </p>
            ...
            <div className="mt-5 space-y-3">
              {urgentTasks.slice(0, 3).map((task) => (
                <button
                  ...
                  className="w-full rounded-2xl bg-white/10 p-3 text-left ring-1 ring-white/10 transition hover:bg-white/15"
                >
      ...
      <Card className="mt-4">
        <CardHeader
          ...
          subtitle="Tabel, Kanban, My Work, Calendar și Approvals. Randare optimizată pe view activ."
          action={
            <button onClick={() => setTaskCreateOpen(true)} className="btn-primary">
              <Plus className="h-4 w-4" />
              Task nou
            ...
            {views.map((view) => {
              ...
              return (
                <button
                  ...
                  className={
                    active
                      ? "inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-4 py-2 text-sm font-black text-white"
                      : "inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-600 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
                  }
                >
        ...
        {activeView === "table" && (
          <TaskListLite tasks={filteredTasks.slice(0, 70)} total={filteredTasks.length} onSelect={setSelectedTask} onStatus={updateTaskStatus} />
        )}

        {activeView === "board" && (
          <BoardLite tasks={filteredTasks.slice(0, 140)} total={filteredTasks.length} onSelect={setSelectedTask} onStatus={updateTaskStatus} />
        )}
        ...
        {activeView === "approvals" && (
          <ApprovalsLite
            tasks={filteredTasks.filter((task) => task.status === "Review / QA" || task.priority === "Urgent" || task.priority === "Critic").slice(0, 20)}
...
function TaskListLite({
  ...
  return (
    <div className="overflow-x-auto">
      {total > tasks.length && (
        <div className="border-b border-slate-100 bg-amber-50 px-5 py-3 text-sm font-semibold text-amber-800">
          Se afișează primele {tasks.length} taskuri din {total}. Folosește filtrele pentru rezultate mai precise.
        </div>
      )}
      ...
      <table className="min-w-full text-left text-sm">
        <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
          <tr>
            ...
            <th className="px-5 py-3">Responsabil</th>
...
function BoardLite({
  ...
  return (
    <div className="p-5">
      {total > tasks.length && (
        <div className="mb-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
          Board optimizat: se afișează primele {tasks.length} taskuri din {total}.
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                <Badge tone={statusTone(status)}>{columnTasks.length}</Badge>
              </div>
              ...
              <div className="space-y-3">
                {visibleTasks.map((task) => (
                  <button
                    ...
                    className="block w-full rounded-[1.25rem] border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-card"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="text-xs font-black text-slate-500">{task.projectCode}</div>
                      <Badge tone={priorityTone(task.priority)}>{task.priority}</Badge>
                    </div>

                    <div className="mt-2 line-clamp-2 text-sm font-black text-slate-950">{task.title}</div>
                    <div className="mt-2 text-xs text-slate-500">{task.assigneeName} · {task.dueDate}</div>
                  </button>
                ))}

                {hidden > 0 && (
                  <div className="rounded-2xl border border-slate-200 bg-white/70 p-3 text-center text-xs font-bold text-slate-500">
                    +{hidden} ascunse pentru performanță
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="rounded-[1.45rem] border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="space-y-3">
        {tasks.slice(0, 8).map((task) => (
          <button key={task.id} onClick={() => onSelect(task.id)} className="w-full rounded-2xl border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:border-emerald-200">
            <div className="font-black text-slate-950">{task.title}</div>
            <div className="mt-1 text-xs text-slate-500">{task.projectCode} · {task.assigneeName}</div>
          </button>
        ))}

        {tasks.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 p-5 text-center text-xs font-bold text-slate-400">Nimic aici.</div>}
      </div>
    </div>
  );
}
...
function CalendarLite({ tasks, onSelect }: { tasks: Task[]; onSelect: (id?: string) => void }) {
  return (
    <div className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7">
      {Array.from({ length: 35 }).map((_, index) => {
        const task = tasks.length ? tasks[index % tasks.length] : undefined;

        return (
          <div key={index} className="min-h-[112px] rounded-2xl border border-slate-200 bg-white p-3">
            <div className="mb-2 text-xs font-black text-slate-500">{index + 1}</div>

            {task && index % 4 === 0 && (
              <button onClick={() => onSelect(task.id)} className="rounded-xl bg-emerald-50 p-2 text-left text-xs font-bold text-emerald-800">
                {task.title}
              </button>
            )}
          </div>
...
function ApprovalsLite({ tasks, onSelect }: { tasks: Task[]; onSelect: (id?: string) => void }) {
  return (
    <div className="space-y-3 p-5">
      {tasks.map((task) => (
        <button key={task.id} onClick={() => onSelect(task.id)} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-emerald-200 hover:bg-emerald-50/40">
          <div>
            <div className="font-black text-slate-950">{task.title}</div>
            <div className="mt-1 text-sm text-slate-500">{task.projectCode} · {task.assigneeName}</div>
          </div>
      ...
      {tasks.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm font-semibold text-slate-500">Nu există aprobări în coadă.</div>}
    </div>
  );
}
