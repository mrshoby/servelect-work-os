...
export default function PrismaTaskReadShadowPage() {
    ...
    <main className="space-y-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-950">v3.2.0 — Prisma Task Read Shadow Verification</h1>
            <p className="mt-3 max-w-5xl text-sm leading-6 text-slate-600">{release.summary}</p>
          </div>
          <span className="rounded-full bg-blue-50 px-4 py-2 text-xs font-black text-blue-700 ring-1 ring-blue-200">
            {release.mode}
          </span>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {release.checks.map((check) => (
          <article key={check.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-3 text-xs font-bold uppercase tracking-wide text-slate-400">source: {check.source}</div>
          </article>
        ))}
      </section>
    </main>
