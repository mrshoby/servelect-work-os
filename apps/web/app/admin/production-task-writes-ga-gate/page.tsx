import { getProductionTaskWritesGaGate } from "@/lib/enterprise/production-task-writes-ga-gate";
...
export default function ProductionTaskWritesGaGatePage() {
    ...
    <main className="space-y-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700 ring-1 ring-emerald-200">
            GA gate: {release.gaGateEnabled ? "ON" : "OFF"}
          </span>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        {release.gates.map((gate) => (
          <article key={gate.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-slate-950">{gate.label}</h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">{gate.requirement}</p>
              </div>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-emerald-500" style={{ width: `${gate.readiness}%` }} />
              </div>
            </div>
            <p className="mt-3 text-xs font-semibold text-slate-500">Evidence: {gate.evidence}</p>
          </article>
        ))}
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-xl font-black text-slate-950">GA rollout waves</h2>
          <div className="mt-4 space-y-3">
            {release.rolloutWaves.map((wave) => (
              <div key={wave.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-2 text-sm text-slate-600">{wave.scope}</p>
                <p className="mt-2 text-xs font-semibold text-slate-500">Rollback window: {wave.rollbackWindow}</p>
        ...
        <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-xl font-black text-slate-950">Rollback controls</h2>
          <div className="mt-4 space-y-3">
            {release.rollbackControls.map((control) => (
              <div key={control.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-2 text-sm text-slate-600">Response: {control.response}</p>
              </div>
            ))}
          </div>
        </article>
      ...
      <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-xl font-black text-slate-950">Go / no-go checklist</h2>
        <ul className="mt-4 grid gap-3 md:grid-cols-2">
          {release.goNoGoChecklist.map((item) => (
