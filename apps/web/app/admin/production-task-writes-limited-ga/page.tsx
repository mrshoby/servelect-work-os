...
export default function ProductionTaskWritesLimitedGaPage() {
  ...
  return (
    <main className="space-y-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700 ring-1 ring-emerald-200">
            Limited GA ready: {release.limitedGaReady ? "YES" : "NO"}
          </span>
        </div>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        {release.waves.map((wave) => (
          <article key={wave.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-slate-950">{wave.label}</h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">{wave.audience}</p>
              </div>
              <span className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${toneFor(wave.status)}`}>{wave.status}</span>
            </div>
            <div className="mt-4 flex items-center gap-3">
              <div className="min-w-14 text-sm font-black text-slate-800">{wave.readiness}%</div>
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-emerald-500" style={{ width: `${wave.readiness}%` }} />
              </div>
            </div>
            <p className="mt-3 text-xs font-semibold text-slate-500">Rollback window: {wave.rollbackWindow}</p>
            <p className="mt-2 text-xs font-semibold text-slate-500">Operations: {wave.allowedOperations.join(", ")}</p>
          </article>
        ))}
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-xl font-black text-slate-950">Monitoring controls</h2>
          <div className="mt-4 space-y-3">
            {release.monitoringControls.map((control) => (
              <div key={control.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                ...
                <p className="mt-2 text-sm text-slate-600">{control.signal}</p>
                <p className="mt-2 text-xs font-semibold text-slate-500">Threshold: {control.threshold}</p>
                <p className="mt-1 text-xs font-semibold text-slate-500">Owner: {control.owner}</p>
              </div>
            ))}
          </div>
        </article>
        ...
        <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-xl font-black text-slate-950">Rollback playbooks</h2>
          <div className="mt-4 space-y-3">
            {release.rollbackPlaybooks.map((playbook) => (
              <div key={playbook.id} className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="font-black text-slate-950">{playbook.label}</div>
                  <span className={`rounded-full px-3 py-1 text-xs font-black ring-1 ${toneFor(playbook.status)}`}>{playbook.status}</span>
                </div>
                <p className="mt-2 text-sm text-slate-600">Trigger: {playbook.trigger}</p>
                <p className="mt-2 text-sm text-slate-600">Response: {playbook.response}</p>
                <p className="mt-2 text-xs font-semibold text-slate-500">Recovery target: {playbook.recoveryTarget}</p>
              </div>
            ))}
          </div>
        </article>
      ...
      <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-xl font-black text-slate-950">Limited GA checklist</h2>
        <ul className="mt-4 grid gap-3 md:grid-cols-2">
          {release.limitedGaChecklist.map((item) => (
