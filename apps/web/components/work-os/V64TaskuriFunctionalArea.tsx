...
function KpiStrip({ pageId, tasks, tickets, projects, approvals }: { pageId: V64PageId; tasks: V64Task[]; tickets: V64Ticket[]; projects: V64Project[]; approvals: V64Approval[] }) {
  ...
  const map: Record<V64PageId, Array<{ label: string; value: string; sub: string; tone: string; icon: ReactNode }>> = {
    ...
    "my-work": [
      { label: "Asignate mie", value: String(tasks.filter((t) => t.assigneeId === "u1").length), sub: "din taskuri vizibile", tone: "green", icon: <CalendarDays /> },
      { label: "Create de mine", value: String(tasks.filter((t) => t.createdBy === "u1").length), sub: "din total", tone: "blue", icon: <FileText /> },
    ...
    inbox: [
      ...
      { label: "Delegate", value: String(tasks.filter((t) => t.ownerId === "u1").length), sub: "urmărire", tone: "purple", icon: <Users /> },
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="flex flex-wrap items-center gap-2">
        <Select label="Vederi salvate" value={filters.savedView} onChange={(value) => setFilters((state) => ({ ...state, savedView: value as SavedViewId }))} options={["all", "my-work", "in-progress", "blocked", "today", "week", "no-deadline", "high-priority", "overdue", "waiting-approval"].map((value) => ({ value, label: value === "all" ? "Toate taskurile" : value }))} />
